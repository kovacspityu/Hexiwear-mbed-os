...
mI2C(I2C_SDA, I2C_SCL), mAddress(0xC0), mInterruptOne(PTD12), mInterruptTwo(PTD10)
{
    ...
    write(MPL_CTRL_REG_1, &data);
    read(MPL_CTRL_REG_2, &data);
    data&=0b00110000;
    data|=timeStep;
    write(MPL_CTRL_REG_2, &data);
    if(mode==MPL_ALTIMETER){visAltimeter = true;}
    else{visAltimeter = false;}
    visFIFO = false;
    data = 0b00000111;
    write(MPL_EVENT_CONFIG, &data);
    setActive();
}

void MPL3115A2::setActive(){
    uint8_t data;
    read(MPL_CTRL_REG_1, &data);
    if(data&1){return;}
    else{
        data|=1;
        write(MPL_CTRL_REG_1, &data);
...
float MPL3115A2::getData(){
    ...
    read(MPL_PRESSURE_MSB, data, dataLength);
    float result;
...
float MPL3115A2::getTemperature(){
    ...
    if(isFIFO()){
        offset=3;
        address = MPL_PRESSURE_MSB;
    }
    else{
        offset = 0;
        address = MPL_TEMPERATURE_MSB; 
    }
...
float MPL3115A2::getDataDelta(){
    uint8_t data[3];
    read(MPL_PRESSURE_DELTA_MSB, data, 3);
...
float MPL3115A2::getTemperatureDelta(){
    uint8_t data[2];
    read(MPL_TEMPERATURE_DELTA_MSB, data, 2);
    return convertTemperatureI2D(data);
}

float MPL3115A2::getMaxData(){
    uint8_t data[3];
    read(MPL_MAX_PRESSURE_MSB, data, 3);
    if (isAltimeter()){return convertAltitudeI2D(data);}
    else{return convertPressureI2D(data);}
}
float MPL3115A2::getMaxTemperature(){
    uint8_t data[2];
    read(MPL_MAX_TEMPERATURE_MSB, data, 2);
    return convertTemperatureI2D(data);
}
float MPL3115A2::getMinData(){
    uint8_t data[3];
    read(MPL_MIN_PRESSURE_MSB, data, 3);
    if (isAltimeter()){return convertAltitudeI2D(data);}
    else{return convertPressureI2D(data);}
}
...
void MPL3115A2::setSeaLevelPressure(float pressure){
    ...
    if(pressure){
        *data = (uint16_t) lround(pressure/2);
        write(MPL_PRESSURE_REFERENCE_MSB, data, 2);
    }
    else{
        if(isAltimeter()){
            uint8_t dummy;
            read(MPL_CTRL_REG_1, &dummy);
            dummy|=0b00000010;
            write(MPL_CTRL_REG_1, &dummy);
            setMode(MPL_BAROMETER);
...
//TODO Check if the underlying bits are actually left untouched, as they should.
    ...
    write(MPL_PRESSURE_OFFSET, &upressure); 
}
...
//TODO Right now it lacks functionality, the hardware can interrupt on just new pressure/altitude or new temperature
        ...
        case I_PRESSURE_CHANGE_MPL : {
            if(activeInterruptsOne&(1<<5)||activeInterruptsTwo&(1<<5)){removeInterrupt(I_PRESSURE_WINDOW_MPL);}
            uint8_t data[2];
            if(isAltimeter()){
                if(target){
                    convertAltitudeD2I(target, data);
                    write(MPL_ALTITUDE_WINDOW_MSB, data, 2);
                }
                else{
                    uint8_t dummy;
                    read(MPL_CTRL_REG_2, &dummy);
                    dummy |= 0b00110000;
                    write(MPL_CTRL_REG_2, &dummy);
                }
                break;
            }
            else{
                if(target){
                    ...
                    write(MPL_PRESSURE_WINDOW_MSB, data, 2);
                }
                else{
                    uint8_t dummy;
                    read(MPL_CTRL_REG_2, &dummy);
        ...
        case I_TEMPERATURE_CHANGE_MPL : {
            if(activeInterruptsOne&(1<<4)||activeInterruptsTwo&(1<<4)){removeInterrupt(I_TEMPERATURE_WINDOW_MPL);}
            uint8_t data;
            if(target){
                data = convertTemperatureD2I(target);
                write(MPL_TEMPERATURE_WINDOW, &data);
            }
            else{
                read(MPL_CTRL_REG_2, &data);
                data |= 0b00110000;
        ...
        case I_ALTITUDE_THRESHOLD_MPL : {
            if(isAltimeter()){
                uint8_t data[2];
                if(target){
                    convertAltitudeD2I(target, data); 
                    write(MPL_ALTITUDE_TARGET_MSB, data, 2);
                    }
                else{
                    uint8_t dummy;
                    read(MPL_CTRL_REG_2, &dummy);
                ...
                if(target){
                    uint8_t data[2];
                    convertPressureD2I(target, data);
                    write(MPL_PRESSURE_TARGET_MSB, data, 2);}
                else{
                    read(MPL_CTRL_REG_2, &data);
                    data |= 0b00110000;
                    write(MPL_CTRL_REG_2, &data);
        ...
        case I_TEMPERATURE_THRESHOLD_MPL : {
            if(target){
                uint8_t data = convertTemperatureD2I(target);
                write(MPL_TEMPERATURE_TARGET, &data);}
            else{
                read(MPL_CTRL_REG_2, &data);
                data |= 0b00110000;
                write(MPL_CTRL_REG_2, &data);
        ...
        case I_ALTITUDE_WINDOW_MPL : {
            if(activeInterruptsOne&(1<<1)||activeInterruptsTwo&(1<<1)){removeInterrupt(I_ALTITUDE_CHANGE_MPL);}
            if(isAltimeter()){
                if(target){
                    uint8_t data[2];
                    convertAltitudeD2I(target, data);
        ...
        case I_TEMPERATURE_WINDOW_MPL : {
            if(activeInterruptsOne&1||activeInterruptsTwo&1){removeInterrupt(I_TEMPERATURE_CHANGE_MPL);}
            if(target){
                uint8_t data = convertTemperatureD2I(target);
                write(MPL_TEMPERATURE_WINDOW, &data);
                }
    ...
    read(MPL_CTRL_REG_4, &data);
    data |= name;
    write(MPL_CTRL_REG_4, &data);
    if(pin==MPL_PIN_ONE){activeInterruptsOne |= name;
...
void MPL3115A2::interruptWrapperOne(){
    interruptWrapper(MPL_PIN_ONE);
}
void MPL3115A2::interruptWrapperTwo(){
    interruptWrapper(MPL_PIN_TWO);
}
void MPL3115A2::interruptWrapper(MPL3115A2_Interrupt_Pin pin){
    while(1){
        ...
        switch(identifyInterrupt(pin)){
            case I_ALTITUDE_CHANGE_MPL : ;
            case I_ALTITUDE_THRESHOLD_MPL : ;
            case I_ALTITUDE_WINDOW_MPL : {
                if(isAltimeter()){
                    mail->type = MPL_TYPE_ALTITUDE;
                    }
                else{
                    mail->type = MPL_TYPE_PRESSURE;
                    }
                mail->value = getData();
                mailBox.put(mail);
                break;
            ...
            case I_TEMPERATURE_THRESHOLD_MPL : ;
            case I_TEMPERATURE_WINDOW_MPL : {
                mail->type = MPL_TYPE_TEMPERATURE;
            ...
            case I_FIFO_MPL : {
                ...
                read(MPL_FIFO_STATUS, &samplesNumber);
                samplesNumber&=0x3F;
                ...
                read(MPL_FIFO_DATA, samples, 5*samplesNumber);
                for(int i=0;i<samplesNumber-1;i++){
                    if(isAltimeter()){
                        (*(mailArray+2*i))->type = MPL_TYPE_ALTITUDE;
                        (*(mailArray+2*i))->value = convertAltitudeI2D(samples+5*i);
                    }
                    else{
                        (*(mailArray+2*i))->type = MPL_TYPE_PRESSURE;
                        (*(mailArray+2*i))->value = convertPressureI2D(samples+5*i);
                    }
                    (*(mailArray+2*i+1))->type = MPL_TYPE_TEMPERATURE;
            ...
            case I_NEW_DATA_MPL : {
                switch(data&0xE){
                    case 0xE: {
                        ...
                        mail2->type = MPL_TYPE_TEMPERATURE;
...
void MPL3115A2::dispatchInterruptDataOne(){
    dispatchInterruptData(MPL_PIN_ONE);
}

void MPL3115A2::dispatchInterruptDataTwo(){
    dispatchInterruptData(MPL_PIN_TWO);
}
...
MPL3115A2_Interrupt MPL3115A2::identifyInterrupt(MPL3115A2_Interrupt_Pin pin){
    uint8_t data;
    read(MPL_INTERRUPT_STATUS, &data);
    if(pin==MPL_PIN_ONE){return static_cast<MPL3115A2_Interrupt> (lround(activeInterruptsOne & data));}
