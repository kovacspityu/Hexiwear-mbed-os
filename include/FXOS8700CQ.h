...
class FXOS8700CQ{
    ...
    public:
        ...
        void setFreefallMotion(uint8_t count, bool resetCount, uint8_t config, float threshold, float xThreshold=0, float yThreshold=0, float zThreshold=0);

        float* getAcceleration();
        float* getMagnetic();
        float getTemperature();
        void setInterrupt(FXOS8700CQ_Interrupt_Pin pin, FXOS8700CQ_Interrupt name, void (*function)(), float threshold = 0, uint8_t count = 0, bool resetCount = false, uint8_t config = 0);
